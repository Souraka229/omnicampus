"""
NFL Draft v14 Optimized - Maximum performance
"""
...
# ==================== TRAIN ====================
# 5 CB configs × 4 seeds = 20 models
configs=[
    dict(iterations=1000,depth=5,learning_rate=0.05,l2_leaf_reg=3,border_count=64,bagging_temperature=0.3,random_strength=0.5),
    dict(iterations=1000,depth=6,learning_rate=0.04,l2_leaf_reg=5,border_count=128,bagging_temperature=0.5,random_strength=0.8),
    dict(iterations=1000,depth=4,learning_rate=0.06,l2_leaf_reg=6,border_count=64,bagging_temperature=0.8,random_strength=1.5),
    dict(iterations=1200,depth=5,learning_rate=0.035,l2_leaf_reg=4,border_count=128,bagging_temperature=0.4,random_strength=0.6),
    dict(iterations=800,depth=7,learning_rate=0.03,l2_leaf_reg=7,border_count=64,bagging_temperature=1.0,random_strength=2.0),
]
...
print(f"CB 20: {roc_auc_score(y,cb_oof):.5f}")
...
for s in cb_seeds:
    ...
    for tr_idx,va_idx in cv.split(X_tr,y):
        m=xgb.XGBClassifier(n_estimators=1000,max_depth=4,learning_rate=0.025,subsample=0.75,
...
for s in cb_seeds:
    oof=np.zeros(n_train); tp=np.zeros(len(X_te))
    for tr_idx,va_idx in cv.split(X_tr,y):
        m=lgb.LGBMClassifier(n_estimators=1000,max_depth=5,learning_rate=0.025,num_leaves=31,
