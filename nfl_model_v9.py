"""
NFL Draft v9 - Maximum feature engineering + 5-fold ensemble
(Exact reproduction du code qui donne 0.84+)
"""
...
all_data["Top2_pct"]   = all_data[[f"{c}_pct" for c in PERF]].apply(lambda r: r.nlargest(2).mean(), axis=1)

# RANK WITHIN DRAFT YEAR × POSITION (clé!)
for col in PERF:
...
# RANK WITHIN DRAFT YEAR × POSITION_TYPE
for col in PERF:
...
# Number of players from same school in same year
...
# K-fold target encoding (strict, 5-fold)
...
# ── Models ────────────────────────────────────────────────────────────────────
print("CB-A..."); cba_oof,cba_tp=run_cb(dict(iterations=1000,depth=5,learning_rate=0.05,
    l2_leaf_reg=3,border_count=64,bagging_temperature=0.3,random_strength=0.5))
print(f"  CB-A: {roc_auc_score(y,cba_oof):.5f}")

print("CB-B..."); cbb_oof,cbb_tp=run_cb(dict(iterations=1000,depth=6,learning_rate=0.04,
    l2_leaf_reg=6,border_count=128,bagging_temperature=0.6,random_strength=1.0),seed=43)
print(f"  CB-B: {roc_auc_score(y,cbb_oof):.5f}")

print("CB-C..."); cbc_oof,cbc_tp=run_cb(dict(iterations=1000,depth=4,learning_rate=0.06,
    l2_leaf_reg=8,border_count=64,bagging_temperature=1.0,random_strength=2.0),seed=44)
print(f"  CB-C: {roc_auc_score(y,cbc_oof):.5f}")

print("XGB...")
xgb_oof,xgb_tp=run_sk(xgb.XGBClassifier(n_estimators=800,max_depth=4,learning_rate=0.03,
...
print("LGB...")
lgb_oof,lgb_tp=run_sk(lgb.LGBMClassifier(n_estimators=800,max_depth=5,learning_rate=0.03,
    num_leaves=31,subsample=0.75,colsample_bytree=0.65,min_child_samples=20,
...
print(f"  MLP: {roc_auc_score(y,mlp_oof):.5f}")
...
cands={"lr":(lr_oof,lr_te),"blend":(bl_oof,bl_test)}
best_k=max(cands,key=lambda k:roc_auc_score(y,cands[k][0]))
best_oof,best_test=cands[best_k]
print(f"Best: {best_k} → {roc_auc_score(y,best_oof):.5f}")

# ── Pseudo-labeling ───────────────────────────────────────────────────────────
THRESH=0.87
pmask=(best_test>THRESH)|(best_test<(1-THRESH))
print(f"Pseudo: {pmask.sum()} samples")
if pmask.sum()>20:
    plabels=(best_test[pmask]>0.5).astype(int)
    ...
    for tr_idx,va_idx in cv.split(np.arange(n_train),y):
        ...
        m=CatBoostClassifier(iterations=1000,depth=5,learning_rate=0.05,l2_leaf_reg=3,
    ...
    print(f"Pseudo CB: {p_auc:.5f}")
    if p_auc>roc_auc_score(y,best_oof):
        best_oof=0.5*p_oof+0.5*best_oof; best_test=0.5*p_tp+0.5*best_test

# Mega blend
all_f=[(bl_oof,bl_test),(lr_oof,lr_te),(best_oof,best_test)]
