"""
NFL Draft v15 FAST - Fast but still powerful
"""
...
all_data["has_all_tests"] = all_data["n_missing"]==0
...
all_data["Agility_Sum"] = (all_data["Agility_3cone"].fillna(all_data["Agility_3cone"].median())+all_data["Shuttle"].fillna(all_data["Shuttle"].median()))
...
for col in PERF:
    ...
    all_data[f"{col}_year_z"]=(all_data[col]-yr_mean)/yr_std
    if col in TIME_COLS: all_data[f"{col}_year_z"]*=-1
...
power5={"Alabama","Ohio State","Georgia","Michigan","Notre Dame","Oklahoma","LSU","Florida","USC","Auburn","Penn State","Texas","Oregon","Tennessee","Florida State","Clemson","Nebraska","Wisconsin","Iowa","Michigan State","Stanford","Washington","UCLA","Virginia Tech","Miami","Pittsburgh","TCU","Baylor","Kansas State","Iowa State","Oklahoma State","West Virginia","Texas A&M","Missouri","Arkansas","Mississippi State","South Carolina","Kentucky","Vanderbilt","Mississippi"}
...
all_data["yr_size"] = all_data.groupby("Year")["Id"].transform("count")
...
X_ite=imp2.transform(all_data.iloc[n_train:][all_feat_cols])
print(f"✓ {len(all_feat_cols)} features")

cv=StratifiedKFold(n_splits=N_FOLDS,shuffle=True,random_state=RNG)

# 4 CB configs × 4 seeds = 16 models
configs=[
    dict(iterations=800,depth=5,learning_rate=0.05,l2_leaf_reg=3,border_count=64),
    dict(iterations=800,depth=6,learning_rate=0.04,l2_leaf_reg=5,border_count=128),
    dict(iterations=800,depth=4,learning_rate=0.06,l2_leaf_reg=6,border_count=64),
    dict(iterations=1000,depth=5,learning_rate=0.035,l2_leaf_reg=4,border_count=128),
]
cb_seeds=[42,123,456,789]

all_cb_oofs=[]; all_cb_tps=[]
for ci,cfg in enumerate(configs):
    for s in cb_seeds:
        oof=np.zeros(n_train); tp=np.zeros(len(X_ite))
...
print(f"CB 16: {roc_auc_score(y,cb_oof):.5f}")
...
for s in cb_seeds:
    ...
    for tr_idx,va_idx in cv.split(X_tr,y):
        m=xgb.XGBClassifier(n_estimators=800,max_depth=4,learning_rate=0.03,subsample=0.75,colsample_bytree=0.65,min_child_weight=5,gamma=0.1,reg_alpha=0.3,reg_lambda=2.0,use_label_encoder=False,eval_metric="auc",random_state=s,n_jobs=-1,verbosity=0)
        m.fit(X_tr[tr_idx],y[tr_idx])
        oof[va_idx]=m.predict_proba(X_tr[va_idx])[:,1]
        tp+=m.predict_proba(X_ite)[:,1]/N_FOLDS
    xgb_oofs.append(oof); xgb_tps.append(tp)
xgb_oof=np.mean(xgb_oofs,axis=0); xgb_tp=np.mean(xgb_tps,axis=0)
print(f"XGB 4: {roc_auc_score(y,xgb_oof):.5f}")
...
for s in cb_seeds:
    oof=np.zeros(n_train); tp=np.zeros(len(X_ite))
    for tr_idx,va_idx in cv.split(X_tr,y):
        m=lgb.LGBMClassifier(n_estimators=800,max_depth=5,learning_rate=0.03,num_leaves=31,subsample=0.75,colsample_bytree=0.65,min_child_samples=20,reg_alpha=0.3,reg_lambda=2.0,random_state=s,n_jobs=-1,verbose=-1)
        m.fit(X_tr[tr_idx],y[tr_idx])
        oof[va_idx]=m.predict_proba(X_tr[va_idx])[:,1]
        tp+=m.predict_proba(X_ite)[:,1]/N_FOLDS
    lgb_oofs.append(oof); lgb_tps.append(tp)
lgb_oof=np.mean(lgb_oofs,axis=0); lgb_tp=np.mean(lgb_tps,axis=0)
print(f"LGB 4: {roc_auc_score(y,lgb_oof):.5f}")
...
lr_oof=np.zeros(n_train); lr_te=np.zeros(len(X_ite))
...
if pmask.sum()>30:
    ...
    p_oofs=[]; p_tps=[]
    for s in cb_seeds:
        p_oof=np.zeros(n_train); p_tp=np.zeros(len(X_ite))
        for tr_idx,va_idx in cv.split(np.arange(n_train),y):
    ...
    print(f"Pseudo: {p_auc:.5f} ({pmask.sum()})")
